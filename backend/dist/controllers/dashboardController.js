...
class DashboardController {
    ...
    static async getDashboardSummary(req, res) {
        try {
            ...
            const dashboardData = await dashboard_service_1.DashboardService.getDashboardSummary(userId);
     ...
     */
    static async getAccountSummaries(req, res) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                res.status(401).json({ error: 'User not authenticated' });
            ...
            const accounts = await dashboard_service_1.DashboardService.getAccountSummaries(userId);
            res.json(accounts);
        }
        catch (error) {
    ...
    static async getRecentTransactions(req, res) {
        try {
            ...
            const transactions = await dashboard_service_1.DashboardService.getRecentTransactions(userId, limit);
    ...
    static async getGoalProgress(req, res) {
        try {
            ...
            const goals = await dashboard_service_1.DashboardService.getGoalProgress(userId);
            res.json(goals);
        }
        catch (error) {
     ...
     */
    static async getSpendingAnalytics(req, res) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                res.status(401).json({ error: 'User not authenticated' });
            ...
            const analytics = await dashboard_service_1.DashboardService.getSpendingAnalytics(userId, period);
            res.json(analytics);
        }
        catch (error) {
    ...
    static async exportData(req, res) {
        try {
            ...
            const exportData = await dashboard_service_1.DashboardService.exportData(userId, type, period, format);
    ...
    static async getQuickSummary(req, res) {
        try {
            ...
            const [accounts, recentTransactions] = await Promise.all([
                dashboard_service_1.DashboardService.getAccountSummaries(userId),
                dashboard_service_1.DashboardService.getRecentTransactions(userId, 5)
            ]);
    ...
    static async getDashboardStats(req, res) {
        try {
            ...
            const [accounts, analytics, goals] = await Promise.all([
                dashboard_service_1.DashboardService.getAccountSummaries(userId),
                dashboard_service_1.DashboardService.getSpendingAnalytics(userId, period),
                dashboard_service_1.DashboardService.getGoalProgress(userId)
            ]);
            const stats = {
