...
class BudgetService {
    ...
    async getBudgetAnalysis(userId, period = 'month', year) {
      ...
      `)
        ...
        const { data: budgetLimits, error: limitsError } = await supabase_1.default
            .from('budget_limits')
            .select('*')
            .eq('user_id', userId)
        ...
        const categoryBreakdown = await this.calculateCategoryBreakdown(transactions || [], budgetLimits || [], totalExpenses);
        // Get trends
        ...
        const recommendations = this.generateBudgetRecommendations(categoryBreakdown, trends, totalIncome, totalExpenses);
        return {
    ...
    async getBudgetTrends(userId, period = 'month', months = 6) {
        const trends = [];
        ...
        for (let i = 0; i < months; i++) {
            ...
            const { data: transactions } = await supabase_1.default
                .from('transactions')
                .select(`
          *,
          categories (id, name)
    ...
    async getCategoryBreakdown(userId, period = 'month') {
        ...
        const { data: transactions, error } = await supabase_1.default
            .from('transactions')
            .select(`
        *,
        categories (id, name, color)
      `)
        ...
        // Get budget limits
        const { data: budgetLimits } = await supabase_1.default
            .from('budget_limits')
            .select('*')
            .eq('user_id', userId)
            .eq('period', period === 'week' ? 'weekly' : period === 'year' ? 'yearly' : 'monthly');
    ...
    async createBudgetLimit(userId, categoryId, limitAmount, period) {
        const { start_date, end_date } = BudgetLimit_1.BudgetLimit.generatePeriodDates(period);
        ...
        return new BudgetLimit_1.BudgetLimit(data);
    }
    async getBudgetLimits(userId) {
        ...
        return data?.map(limit => new BudgetLimit_1.BudgetLimit(limit)) || [];
    }
    async updateBudgetLimit(limitId, limitAmount, period) {
        const updateData = { limit_amount: limitAmount, updated_at: new Date().toISOString() };
        if (period) {
            const { start_date, end_date } = BudgetLimit_1.BudgetLimit.generatePeriodDates(period);
    ...
    // Get budget warnings
    async getBudgetWarnings(userId) {
    ...
    async exportBudgetReport(userId, type, period, format) {
        let reportData;
        switch (type) {
            case 'spending':
    ...
    async calculateCategoryBreakdown(transactions, budgetLimits, totalExpenses) {
        ...
        const categoryBreakdown = Array.from(categoryMap.entries()).map(([categoryId, data]) => {
            ...
            if (data.limit_amount) {
                if (percentage_of_limit >= 100)
                    status = 'over_limit';
                else if (percentage_of_limit >= 75)
    ...
    async calculateCategoryTrends(transactions, previousTrend) {
        const categoryMap = new Map();
        transactions.forEach(transaction => {
            if (transaction.is_expense && transaction.categories) {
                const categoryId = transaction.categories.id;
    ...
    generateBudgetWarnings(categoryBreakdown, currentDate) {
        const warnings = [];
    ...
    generateBudgetRecommendations(categoryBreakdown, trends, totalIncome, totalExpenses) {
    ...
    getPeriodDates(period, year, baseDate) {
...
//# sourceMappingURL=budget_service.js.map