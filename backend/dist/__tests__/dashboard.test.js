...
const dashboard_service_1 = require("../services/dashboard_service");
...
const mockSupabase = supabase_1.default;
const MockedCategoryRepository = category_repository_1.CategoryRepository;
describe('DashboardService', () => {
    ...
    describe('getAccountSummaries', () => {
        it('should return account summaries successfully', async () => {
            ...
            const result = await dashboard_service_1.DashboardService.getAccountSummaries(mockUserId);
        ...
        it('should handle database errors', async () => {
            ...
            mockSupabase.from.mockReturnValue(mockSupabaseChain);
            await expect(dashboard_service_1.DashboardService.getAccountSummaries(mockUserId))
    ...
    describe('getRecentTransactions', () => {
        it('should return recent transactions with details', async () => {
            ...
            mockSupabase.from.mockReturnValue(mockSupabaseChain);
            MockedCategoryRepository.findByUserId.mockResolvedValue([mockCategory]);
            const result = await dashboard_service_1.DashboardService.getRecentTransactions(mockUserId, 10);
    ...
    describe('getGoalProgress', () => {
        it('should return goal progress with calculated percentages', async () => {
            ...
            mockSupabase.from.mockReturnValue(mockSupabaseChain);
            const result = await dashboard_service_1.DashboardService.getGoalProgress(mockUserId);
            expect(result).toHaveLength(1);
            expect(result[0]).toMatchObject({
                id: 'goal-1',
                name: 'Emergency Fund',
    ...
    describe('getSpendingAnalytics', () => {
        it('should return comprehensive analytics', async () => {
            ...
            mockSupabase.from.mockReturnValue(mockSupabaseChain);
            const result = await dashboard_service_1.DashboardService.getSpendingAnalytics(mockUserId, 'month');
    ...
    describe('exportData', () => {
        it('should export spending data as CSV', async () => {
            ...
            mockSupabase.from.mockReturnValue(mockSupabaseChain);
            const result = await dashboard_service_1.DashboardService.exportData(mockUserId, 'spending', 'month', 'csv');
        ...
        it('should export data as JSON', async () => {
            ...
            mockSupabase.from.mockReturnValue(mockSupabaseChain);
            const result = await dashboard_service_1.DashboardService.exportData(mockUserId, 'accounts', 'month', 'json');
        ...
        it('should handle invalid export type', async () => {
            await expect(dashboard_service_1.DashboardService.exportData(mockUserId, 'invalid', 'month', 'csv'))
    ...
    describe('getDashboardSummary', () => {
        it('should return complete dashboard summary', async () => {
            ...
            const mockTransactions = [{ id: 'trans-1', amount: 25, type: 'expense', description: 'Test', category: { id: 'cat-1', name: 'Food', color: '#FF0000' }, account: { id: 'acc-1', name: 'Test Account', type: 'checking' }, date: new Date() }];
            const mockGoals = [{ id: 'goal-1', name: 'Test Goal', type: 'savings', target_amount: 1000, current_amount: 500, progress_percentage: 50, days_remaining: 30 }];
            ...
            jest.spyOn(dashboard_service_1.DashboardService, 'getAccountSummaries').mockResolvedValue(mockAccounts);
            jest.spyOn(dashboard_service_1.DashboardService, 'getRecentTransactions').mockResolvedValue(mockTransactions);
            jest.spyOn(dashboard_service_1.DashboardService, 'getGoalProgress').mockResolvedValue(mockGoals);
            jest.spyOn(dashboard_service_1.DashboardService, 'getSpendingAnalytics').mockResolvedValue(mockAnalytics);
            const result = await dashboard_service_1.DashboardService.getDashboardSummary(mockUserId);
...
describe('CategoryRepository', () => {
    ...
    describe('findCategoriesWithSpending', () => {
        it('should return categories with spending data', async () => {
            ...
            category_repository_1.CategoryRepository.findCategoriesWithSpending.mockRestore();
            const startDate = new Date('2024-01-01');
            const endDate = new Date('2024-01-31');
            const result = await category_repository_1.CategoryRepository.findCategoriesWithSpending('user-id', startDate, endDate);
            expect(result).toHaveLength(1);
            expect(result[0]).toMatchObject({
                category_id: 'cat-1',
                category_name: 'Food & Dining',
