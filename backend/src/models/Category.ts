export interface CategoryProps {
  id?: string; // UUID from database, optional for new categories
  user_id: string; // UUID reference to auth.users
  name: string;
  color?: string; // Hex color code, e.g., '#FF5733'
  icon?: string; // Icon identifier, e.g., 'shopping-cart', 'home'
...
export class Category {
  ...
  constructor({ id, user_id, name, color, icon, is_default = false }: CategoryProps) {
    // Validation
  ...
  getContrastingTextColor(): string {
    if (!this.color) return '#000000'; // Default to black if no color
  ...
  static validate(props: CategoryProps): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
