export interface GoalProps {
  ...
  target_date: Date | string; // Required target date per story requirements
  completed?: boolean; // Whether goal has been completed, defaults to false
  created_at?: Date | string; // Database timestamp, optional for new goals
  updated_at?: Date | string; // Database timestamp, optional for new goals
}
...
export class Goal {
  ...
  constructor({ id, user_id, name, type, target_amount, current_amount = 0, target_date, completed = false, created_at, updated_at }: GoalProps) {
    // Validation
    if (!user_id) throw new Error('user_id is required');
    if (!name || name.trim().length === 0) throw new Error('name is required and cannot be empty');
    if (!type || !['savings', 'debt'].includes(type)) throw new Error('type must be either "savings" or "debt"');
  ...
  isOverdue(): boolean {
    if (!this.target_date || this.completed) return false;
    return new Date() > this.target_date;
  }

  getDaysRemaining(): number | null {
    if (!this.target_date || this.completed) return null;
  ...
  static validate(props: GoalProps): { isValid: boolean; errors: string[] } {
    ...
    if (!props.type || !['savings', 'debt'].includes(props.type)) errors.push('type must be either "savings" or "debt"');
