...
export class BudgetController {
  ...
  async getBudgetTrends(req: Request, res: Response) {
    try {
      const userId = req.userId;
  ...
  async getCategoryBreakdown(req: Request, res: Response) {
    try {
      ...
      const categories = await budgetService.getCategoryBreakdown(userId, period as string);
  ...
  async createBudgetLimit(req: Request, res: Response) {
    try {
      const userId = req.userId;
      ...
      const budgetLimit = await budgetService.createBudgetLimit(
        userId,
        category_id,
  ...
  async getBudgetLimits(req: Request, res: Response) {
    try {
      const userId = req.userId;
      ...
      const limits = await budgetService.getBudgetLimits(userId);
  ...
  async updateBudgetLimit(req: Request, res: Response) {
    try {
      const { id } = req.params;
      ...
      const updatedLimit = await budgetService.updateBudgetLimit(
        id,
        parseFloat(limit_amount),
        period
      );
  ...
  async deleteBudgetLimit(req: Request, res: Response) {
    try {
      const { id } = req.params;
  ...
  async getBudgetWarnings(req: Request, res: Response) {
    try {
      const userId = req.userId;
      ...
      const warnings = await budgetService.getBudgetWarnings(userId);
  ...
  async exportBudgetReport(req: Request, res: Response) {
    try {
      const userId = req.userId;
      ...
      if (!['spending', 'limits', 'trends'].includes(type as string)) {
        return res.status(400).json({
          success: false,
      ...
      const exportData = await budgetService.exportBudgetReport(
        userId,
        type as string,
        period as string,
