...
const mockGoalRepository = GoalRepository as jest.Mocked<typeof GoalRepository>;
...
describe('GoalService Tests', () => {
  ...
  describe('createGoal', () => {
    test('should create a valid goal', async () => {
      const goalData: Partial<GoalProps> = {
        name: 'Emergency Fund',
        type: 'savings',
        target_amount: 5000,
      ...
      const result = await GoalService.createGoal('user-123', goalData);
    ...
    test('should throw error for invalid goal data', async () => {
      const invalidGoalData: Partial<GoalProps> = {
        name: '',
        type: 'savings',
        target_amount: -100,
        target_date: '2024-12-31'
      ...
      await expect(
        GoalService.createGoal('user-123', invalidGoalData)
      ).rejects.toThrow('Goal validation failed');
  ...
  describe('getGoals', () => {
    test('should retrieve goals with filters', async () => {
      ...
      const filters = { type: 'savings' as const };
      const result = await GoalService.getGoals('user-123', filters);
  ...
  describe('getGoalById', () => {
    ...
    test('should throw error when goal not found', async () => {
      ...
      await expect(
        GoalService.getGoalById('nonexistent-goal', 'user-123')
  ...
  describe('updateGoal', () => {
    test('should update goal with valid data', async () => {
      ...
      const result = await GoalService.updateGoal('goal-123', 'user-123', updates);
    ...
    test('should throw error for invalid updates', async () => {
      ...
      await expect(
        GoalService.updateGoal('goal-123', 'user-123', invalidUpdates)
      ).rejects.toThrow('Goal validation failed');
  ...
  describe('updateGoalProgress', () => {
    test('should update goal progress correctly', async () => {
      ...
      const result = await GoalService.updateGoalProgress('goal-123', 'user-123', 2500);
    ...
    test('should mark goal as completed when target is reached', async () => {
      ...
      const result = await GoalService.updateGoalProgress('goal-123', 'user-123', 5000);
    ...
    test('should throw error for negative progress', async () => {
      ...
      await expect(
        GoalService.updateGoalProgress('goal-123', 'user-123', -100)
  ...
  describe('markGoalCompleted', () => {
    test('should mark goal as completed', async () => {
      ...
      const result = await GoalService.markGoalCompleted('goal-123', 'user-123');
  ...
  describe('deleteGoal', () => {
    test('should delete goal successfully', async () => {
      ...
      const result = await GoalService.deleteGoal('goal-123', 'user-123');
  ...
  describe('getGoalAnalytics', () => {
    test('should calculate goal analytics correctly', async () => {
      ...
      const result = await GoalService.getGoalAnalytics('user-123');
  ...
  describe('calculateDailySavingsNeeded', () => {
    test('should calculate daily savings needed correctly', () => {
      ...
      const dailySavings = GoalService.calculateDailySavingsNeeded(goal);
  ...
  describe('type-specific goal retrieval', () => {
    test('should get savings goals', async () => {
      ...
      const result = await GoalService.getSavingsGoals('user-123');
    ...
    test('should get debt goals', async () => {
      ...
      const result = await GoalService.getDebtGoals('user-123');
    ...
    test('should get active goals', async () => {
      ...
      const result = await GoalService.getActiveGoals('user-123');
