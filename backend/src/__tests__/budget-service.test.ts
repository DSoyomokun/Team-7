...
describe('BudgetService', () => {
  ...
  describe('createBudgetLimit', () => {
    ...
    it('should create a new budget limit', async () => {
      ...
      const result = await budgetService.createBudgetLimit(
        mockUserId,
        mockCategoryId,
        mockLimitAmount,
        mockPeriod
      ...
      expect(result).toBeInstanceOf(BudgetLimit);
    ...
    it('should handle creation errors', async () => {
      ...
      await expect(budgetService.createBudgetLimit(
        mockUserId,
        mockCategoryId,
        mockLimitAmount,
        mockPeriod
      )).rejects.toThrow('Failed to create budget limit: Creation failed');
    });
  });
  ...
  describe('getBudgetLimits', () => {
    ...
    it('should return all budget limits for user', async () => {
      ...
      expect(result[0]).toBeInstanceOf(BudgetLimit);
      expect(result[1]).toBeInstanceOf(BudgetLimit);
    });
  ...
  describe('exportBudgetReport', () => {
    it('should export spending report in JSON format', async () => {
      ...
      const result = await budgetService.exportBudgetReport(
        mockUserId,
        'spending',
        'month',
        'json'
    ...
    it('should export spending report in CSV format', async () => {
      ...
      jest.spyOn(budgetService, 'getBudgetAnalysis').mockResolvedValue(mockAnalysis as any);
    ...
    it('should throw error for invalid report type', async () => {
      await expect(budgetService.exportBudgetReport(
        mockUserId,
        'invalid',
        'month',
        'json'
      ...
      const mockCategoryBreakdown = [
        {
          ...
          status: 'approaching_limit' as const,
          color: '#FF6B6B'
        }
      ];
