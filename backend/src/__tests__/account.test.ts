...
describe('Account Model', () => {
  const validAccountProps: AccountProps = {
    user_id: 'user123',
  ...
  describe('Constructor', () => {
    it('should create account with valid props', () => {
      const account = new Account(validAccountProps);
    ...
    it('should throw error for missing user_id', () => {
      ...
      expect(() => new Account(props)).toThrow('user_id is required');
    });

    it('should throw error for empty name', () => {
      const props = { ...validAccountProps, name: '' };
      expect(() => new Account(props)).toThrow('name is required and cannot be empty');
    });

    it('should throw error for empty type', () => {
      const props = { ...validAccountProps, type: '' };
      expect(() => new Account(props)).toThrow('type is required and cannot be empty');
    });

    it('should throw error for invalid balance', () => {
      const props = { ...validAccountProps, balance: NaN };
      expect(() => new Account(props)).toThrow('balance must be a valid number');
    });
  ...
  describe('Balance Operations', () => {
    ...
    beforeEach(() => {
      account = new Account({ ...validAccountProps, balance: 1000 });
    });
  ...
  describe('Balance Utilities', () => {
    ...
    it('should check if account can afford amount', () => {
      const account = new Account({ ...validAccountProps, balance: 500 });
  ...
  describe('Validation', () => {
    it('should validate correct props', () => {
      const result = Account.validate(validAccountProps);
    ...
    it('should validate missing user_id', () => {
      const props = { ...validAccountProps, user_id: '' };
      const result = Account.validate(props);
  ...
  describe('Static Methods', () => {
    it('should return valid account types', () => {
      const types = Account.getValidTypes();
